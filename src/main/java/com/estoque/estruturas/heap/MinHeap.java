 ...
 * ESTOQUE: produtos com estoque mais baixo "sobem"
 * 
 * A implementacao mantem um mapa auxiliar (item -> indice no array)
...
public class MinHeap<T> {
    ...
    private final Map<T, Integer> indices; // item -> posicao no array
    ...
    // Remove um item especifico da heap (nao precisa ser o minimo). Custo O(log n)
    public boolean remover(T item) {
        ...
        if (i != ultimoIndice) {
            ...
            // pode precisar subir ou descer dependendo da comparacao com o pai e os filhos
            subir(i);
    ...
    // Atualiza a prioridade de um item na heap (sift-up ou sift-down). Custo O(log
    // n)
    ...
    // Retorna, sem alterar a heap, os k elementos de maior prioridade (menor valor)
    // ordenados. Custo O(n log n) - usa uma copia.
    ...
    // Heapsort: esvazia uma copia da heap e retorna os elementos em ordem crescente
    // de prioridade
